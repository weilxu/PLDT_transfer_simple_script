...
for idx in range(0,len(site_df['hostname'])):
    ...
    print(bundle)
    ipv4 = site_df['ipv4'][idx]
    ipv4 = ipv4.split(' ')
    ipv4 = ipv4[0]
    ipv4 = f"{ipv4}/30"
    print(ipv4)
    ipv6 = site_df['ipv6'][idx]
    print(ipv6)
    ...
    while True:
        try:
            response = requests.get(f'http://pldt-tnt-inventory.cisco.com/api/dcim/interfaces/?device={name}&name={bundle}', headers=headers)
